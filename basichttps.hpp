...
protected:
	...
	std::string uri_encode(std::string target);
	std::string m_token;
public:
	...
	http::response<http::dynamic_body> request(std::string target, const std::string &payload, std::string content_type, const http::verb &method);
